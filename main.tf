...
// Create a VPC named "techtest_vpc"
resource "aws_vpc" "techtest_vpc" {
  ...
  // We are tagging the VPC with the name "techtest_vpc"
  tags = {
    Name = "techtest_vpc"
  }
}

// Create an internet gateway named "techtest_igw"
// and attach it to the "techtest_vpc" VPC
resource "aws_internet_gateway" "techtest_igw" {
  // Here we are attaching the IGW to the 
  // techtest_vpc VPC
  vpc_id = aws_vpc.techtest_vpc.id

  // We are tagging the IGW with the name techtest_igw
  tags = {
    Name = "techtest_igw"
  }
}
...
resource "aws_subnet" "techtest_public_subnet" {
  ...
  // Put the subnet into the "techtest_vpc" VPC
  vpc_id            = aws_vpc.techtest_vpc.id
  ...
  // We are tagging the subnet with a name of "techtest_public_subnet_" and
  // suffixed with the count
  tags = {
    Name = "techtest_public_subnet_${count.index}"
  }
}

// Create a group of private subnets based on the variable subnet_count.private
resource "aws_subnet" "techtest_private_subnet" {
  // count is the number of resources we want to create
  ...
  // We are tagging the subnet with a name of "techtest_private_subnet_" and
  // suffixed with the count
  tags = {
    Name = "techtest_private_subnet_${count.index}"
  }
}

// Create a public route table named "techtest_public_rt"
resource "aws_route_table" "techtest_public_rt" {
  // Put the route table in the "techtest_vpc" VPC
  vpc_id = aws_vpc.techtest_vpc.id
  ...
  // Gateway "techtest_igw"
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.techtest_igw.id
  }
}

// Here we are going to add the public subnets to the 
// "techtest_public_rt" route table
resource "aws_route_table_association" "public" {
  ...
  // "techtest_public_rt" from above
  route_table_id = aws_route_table.techtest_public_rt.id
  
  // This is the subnet ID. Since the "techtest_public_subnet" is a 
...
// Create a private route table named "techtest_private_rt"
resource "aws_route_table" "techtest_private_rt" {
  // Put the route table in the "techtest_VPC" VPC
  vpc_id = aws_vpc.techtest_vpc.id
...
// route table "techtest_private_rt"
resource "aws_route_table_association" "private" {
  ...
  // Here we are making sure that the route table is
  // "techtest_private_rt" from above
  route_table_id = aws_route_table.techtest_private_rt.id
  
  // This is the subnet ID. Since the "techtest_private_subnet" is a
  // list of private subnets, we need to use count to grab the
  // subnet element and then grab the ID of that subnet
  subnet_id      = aws_subnet.techtest_private_subnet[count.index].id
}
...
// Create a db subnet group named "techtest_db_subnet_group"
resource "aws_db_subnet_group" "techtest_db_subnet_group" {
  // The name and description of the db subnet group
  name        = "techtest_db_subnet_group"
  description = "DB subnet group for techtest"
  
  // Since the db subnet group requires 2 or more subnets, we are going to
  // loop through our private subnets in "techtest_private_subnet" and
  // add them to this db subnet group
  subnet_ids  = [for subnet in aws_subnet.techtest_private_subnet : subnet.id]
}

// Create a DB instance called "techtest_database"
resource "aws_db_instance" "techtest_database" {
  ...
  // settings.database.db_name variable, which is set to "techtest"
  ...
  // This is the DB subnet group "techtest_db_subnet_group"
  db_subnet_group_name   = aws_db_subnet_group.techtest_db_subnet_group.id
  ...
  // "techtest_db_sg" security group
  vpc_security_group_ids = [aws_security_group.techtest_db_sg.id]
...
// Create a key pair named "techtest_kp"
resource "aws_key_pair" "techtest_kp" {
...
// Create an EC2 instance named "techtest_web"
resource "aws_instance" "techtest_web" {
  ...
  // The subnet ID for the EC2 instance. Since "techtest_public_subnet" is a list
  ...
  // "techtest_public_subnet" and putting the EC2 instance in there
  subnet_id              = aws_subnet.techtest_public_subnet[count.index].id
  
  // The key pair to connect to the EC2 instance. We are using the "techtest_kp" key 
  // pair that we created
  key_name               = aws_key_pair.techtest_kp.key_name
  
  // The security groups of the EC2 instance. This takes a list, however we only
  // have 1 security group for the EC2 instances.
  vpc_security_group_ids = [aws_security_group.techtest_web_sg.id]

  // We are tagging the EC2 instance with the name "techtest_db_" followed by
  // the count index
  tags = {
    Name = "techtest_web_${count.index}"
  }
}

// Create an Elastic IP named "techtest_web_eip" for each
// EC2 instance
resource "aws_eip" "techtest_web_eip" {
  ...
  count    = var.settings.web_app.count
	...
	// The EC2 instance. Since techtest_web is a list of 
  ...
  instance = aws_instance.techtest_web[count.index].id

	// We want the Elastic IP to be in the VPC
  vpc      = true

	// Here we are tagging the Elastic IP with the name
	// "techtest_web_eip_" followed by the count index
  tags = {
    Name = "techtest_web_eip_${count.index}"
  }
}
